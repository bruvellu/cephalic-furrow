...
The total energy per unit length of this tissue ($W_T$) is a sum of a stretching energy component ($W_s$) and a bending energy component ($W_b$) ({@fig:model}c).
...
<!--TODO: Move? There’s no computational analysis here-->
...
Without the germ band, the presence of an active invagination at the head--trunk region reduced the variability of ectopic folding in the neighborhood of the cephalic furrow ({@fig:model}g).
This influence of the cephalic furrow on ectopic folds correlates with the strength of the pull, with higher values preventing ectopic folds more effectively ({@fig:model}g,h, {@fig:cf-sweep}b).
Curiously, this buffering effect diminished with the extension of the germ band for intermediate values of $\kappa_o^{CF}$ ({@fig:cf-sweep}b).
The cephalic furrow becomes shallower and there is an increase in ectopic folds at the posterior region.
Only larger $\kappa_o^{CF}$ values remained effective in preventing ectopic folds at high percentages of germ band extension ({@fig:cf-sweep}b).
...
Finally, given that in wildtype embryos the cephalic furrow forms around 15 min before mitotic domains, we asked whether their relative timing of formation may influence the effectiveness of the cephalic furrow.
Indeed, when we added a delay to the formation of mitotic domains, the cephalic furrow is more effective in preventing ectopic folding even for lower $\kappa_o^{CF}$ values and for greater percentages of germ band extension ({@fig:model}g,h, {@fig:cf-sweep}b).
...
Taken together, our physical model provides a theoretical basis that an early pre-patterned invagination can effectively absorb compressive stresses in the tissue preventing mechanical instabilities in the embryo epithelium during gastrulation.
...
### Novel *btd* head--trunk domain associated with cephalic furrow evolution

<!--Head--trunk boundary slp/eve expression is conserved-->

<!--### Clogmia lacks the head--trunk domain of btd-->

To uncover the genetic patterning differences associated with the evolution of the cephalic furrow, we compared the expression data of cephalic furrow genes in *Drosophila* described above to *Clogmia*, a non-cyclorraphan fly without the cephalic furrow.
...
In the case of the cephalic furrow compressive stresses both generated ectopic buckling and may have had a detrimental effect, being potentially a selective pressure, and secondarily, the gene expression reshuffling was coopted and fixed, eventually establishing the cephalic furrow developmental program.
We propose that this may be a more common and broadly occurring mechanism which may explain the incredible diversity of morphogenetic processes during early embryonic development.
...
For laser ablations, we used a *moe-GFP* line (gift from Eli Knust).
...
The code for the model and simulations will be available in a Zenodo repository [@Krishna2023-hu].
...
**f**, Mutant parameter sweep with mitotic domains (no cephalic furrow). The phase diagram shows an increase in number of folds in relation to **e**. The addition of mitotic domains induces the formation of ectopic folds even without germ band extension (iii).
