...
<!--The value of $K_{b}^{\text{*}} \approx 3.8 \times 10^{-5}$ is lower than our reference bending rigidity.-->
<!--This mismatch, however, may be explained by the difference in thickness of MDCK (~18µm) and blastoderm (~40µm) monolayers.-->
<!--Given that in elastic sheets $K_{b}/K_{s} \propto h^2$ [@Efrati2009-ve], where $h$ is the height of the epithelium, the thickness-adjusted value for MDCK [@Fouchard2020-km] is $K_{b}^{\text{*}} \approx 1.8 \times 10^{-4}$.-->
<!--This is higher but in the same order of magnitude of our reference bending rigidity, indicating that our model, albeit simple, still captures a parameter space consistent with direct measurements.-->
...
When both mitotic domains and germ band are present *in silico*, there is an increase in tissue compression that maximizes the number and speed of ectopic fold formation ({@fig:model-features}b,c).
