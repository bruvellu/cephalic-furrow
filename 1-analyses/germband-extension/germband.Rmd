...
ggsave("2-plots/gb_time_tf_buck_md_start.png", width = 12, height = 4)
ggsave("2-plots/gb_time_tf_buck_md_start.svg", width = 12, height = 4)
```
...
ggsave("2-plots/gb_pct_tf_buck_md_start.png", width = 12, height = 4)
ggsave("2-plots/gb_pct_tf_buck_md_start.svg", width = 12, height = 4)
```

## Tests

```{r tests}
# TODO: Create the relevant tests for testing relevant differences
wilcox.test(
  gblong[(gblong$status == "tf_buck" & gblong$stock != "prd" & gblong$zygosity == "homozygote"),]$time,
  gblong[(gblong$status == "md_start" & gblong$stock != "prd" & gblong$zygosity == "homozygote"),]$time,
  paired = TRUE
)
```
...
ggplot(gblong[gblong$status %in% c("tf_buck", "md_start"),], aes(status, time, fill = zygosity)) +
  ...
  labs(x = "stocks", y = "time after gastrulation (min)") +
  #facet_grid(cols = vars(zygosity), scales = "free") +
  facet_grid(cols = vars(zygosity), rows = vars(stock), scales = "free") +
  theme_classic(base_family = "DejaVu Sans") +
  theme(text = element_text(size = 35),
        strip.background = element_blank(),
        panel.background = element_rect(colour = "black", size=1)
        )
ggsave("2-plots/gb_time_tf_buck_md_start_by_stock.png", width = 12, height = 12)
ggsave("2-plots/gb_time_tf_buck_md_start_by_stock.svg", width = 12, height = 12)
```