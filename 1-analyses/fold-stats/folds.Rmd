...
## Calculate total folds per side

```{r fold-count-per-side}

# Total folds and folded area per embryo
folds_per_side <- aggregate(
...
folds_per_side <- folds_per_side[order(folds_per_side$Stock, folds_per_side$Genotype, folds_per_side$Type),]

# Save data frame to file
write.csv(folds_per_side, file = "folds_per_side.csv", row.names = FALSE)
```

## Calculate average folds per embryo side

```{r fold-average-per-side}

# Total folds and folded area per embryo
folds_per_embryo <- aggregate(
  Fold ~ Embryo + Stock + Genotype + Type,
  folds_per_side,
  function(x) c(
    mean = mean(x),
    sd = sd(x)
...
folds_per_embryo <- folds_per_embryo[order(folds_per_embryo$Stock, folds_per_embryo$Genotype, folds_per_embryo$Type),]

# Save data frame to file
write.csv(folds_per_embryo, file = "folds_per_embryo.csv", row.names = FALSE)
```

## Calculate average folds per genotype

```{r fold-average-per-side}

# Total folds and folded area per embryo
folds_per_genotype <- aggregate(
  Fold.mean ~ Stock + Genotype + Type,
  folds_per_embryo,
  function(x) c(
    mean = mean(x),
    sd = sd(x)
...
# Makes column names nicer
folds_per_genotype <- do.call(data.frame, folds_per_genotype)
folds_per_genotype <- folds_per_genotype[order(folds_per_genotype$Stock, folds_per_genotype$Genotype, folds_per_genotype$Type),]
