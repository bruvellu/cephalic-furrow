...
area <- na.omit(results[results$Fold == TRUE, ])
...
for (stock in unique(area$Stock)) {
  
  n_total <- length(unique(area[area$Stock == stock, ]$Embryo))
  n_het <- length(unique(area[area$Stock == stock & area$Genotype == "heterozygote", ]$Embryo))
  n_hom <- length(unique(area[area$Stock == stock & area$Genotype == "homozygote", ]$Embryo))
...
areacorr <- aggregate(area$AreaCorr, list(area$Embryo), sum)
colnames(areacorr) <- c("Embryo", "AreaCorr")

# Sum total folded area corrected for CF
areacorr_cf <- aggregate(area[area$Type == "CF",]$AreaCorr, list(area[area$Type == "CF",]$Embryo), sum)
colnames(areacorr_cf) <- c("Embryo", "AreaCorrCF")

# Sum total folded area corrected for EF
areacorr_ef <- aggregate(area[area$Type == "EF",]$AreaCorr, list(area[area$Type == "EF",]$Embryo), sum)
colnames(areacorr_ef) <- c("Embryo", "AreaCorrEF")

# Count number of folds
nfold <- data.frame(table(area$Embryo))
colnames(nfold) <- c("Embryo", "Folds")

# Count number of CF folds
nfold_cf <- data.frame(table(area[area$Type == "CF",]$Embryo))
colnames(nfold_cf) <- c("Embryo", "CFs")

# Count number of EF folds
nfold_ef <- data.frame(table(area[area$Type == "EF",]$Embryo))
colnames(nfold_ef) <- c("Embryo", "EFs")

# Get sample information
embryos <- unique(area[, c("Embryo", "Stock", "Genotype")])
...
print_btd <- sprintf("btd: embryos=%s (het=%s, hom=%s), folds=%s (het=%s, hom=%s)",
        ...
        length(area[area$Stock == "btd",]$Label), # total folds
        length(area[area$Genotype == "heterozygote" & area$Stock == "btd",]$Label), # het folds
        length(area[area$Genotype == "homozygote" & area$Stock == "btd",]$Label) # het folds
        )
...
print_eve <- sprintf("eve: embryos=%s (het=%s, hom=%s), folds=%s (het=%s, hom=%s)",
        ...
        length(area[area$Stock == "eve",]$Label), # total folds
        length(area[area$Genotype == "heterozygote" & area$Stock == "eve",]$Label), # het folds
        length(area[area$Genotype == "homozygote" & area$Stock == "eve",]$Label) # het folds
        )
...
print_prd <- sprintf("prd: embryos=%s (het=%s, hom=%s), folds=%s (het=%s, hom=%s)",
        ...
        length(area[area$Stock == "prd",]$Label), # total folds
        length(area[area$Genotype == "heterozygote" & area$Stock == "prd",]$Label), # het folds
        length(area[area$Genotype == "homozygote" & area$Stock == "prd",]$Label) # het folds
        )

# Print wt embryos
print_wt <- sprintf(" wt: embryos=%s, folds=%s, ant=%s, mid=%s, post=%s",
        length(embryos[embryos$Stock == "wt",]$Embryo), # total embryos
        length(area[area$Stock == "wt",]$Label), # total folds
        length(area[area$Stock == "wt" & area$Position == "ant",]$Label), # anterior folds
        length(area[area$Stock == "wt" & area$Position == "mid",]$Label), # middle folds
        length(area[area$Stock == "wt" & area$Position == "post",]$Label) # posterior folds
        )
...
# Bind row by row
stats <- rbind(stats, print_stats(area, "wt", "CF"))
stats <- rbind(stats, print_stats(area, "wt", "EF"))
stats <- rbind(stats, print_stats(area, "btd", "CF"))
stats <- rbind(stats, print_stats(area, "btd", "EF"))
stats <- rbind(stats, print_stats(area, "eve", "CF"))
stats <- rbind(stats, print_stats(area, "eve", "EF"))
stats <- rbind(stats, print_stats(area, "prd", "CF"))
stats <- rbind(stats, print_stats(area, "prd", "EF"))
...
```{r stats}
#TODO Fix these tests to use Genotype
sink('tests.txt')

print("Testing normality of distribution")
shapiro.test(area[(area$Stock != "prd" & area$Type == "EF" & area$Sample == "wildtype"),]$AreaCorr)
shapiro.test(area[(area$Stock != "prd" & area$Type == "EF" & area$Sample == "mutant"),]$AreaCorr)
print("Testing for difference between samples")
wilcox.test(area[(area$Stock != "prd" & area$Type == "EF" & area$Sample == "wildtype"),]$AreaCorr,
            area[(area$Stock != "prd" & area$Type == "EF" & area$Sample == "mutant"),]$AreaCorr)

print("t-test just to get the difference between sample means")
t.test(area[(area$Stock != "prd" & area$Type == "EF" & area$Sample == "wildtype"),]$AreaCorr,
       area[(area$Stock != "prd" & area$Type == "EF" & area$Sample == "mutant"),]$AreaCorr,
       var.equal = FALSE)
...
```{r calculate-total-area-per-embryo, include=FALSE}

#TODO: Began refactoring the quantifications, but decided to simply use what I have already. I can improve the code later on.
