...
ggplot(total, aes(Stock, AreaCorr, fill = Genotype)) +
  geom_boxplot(show.legend = FALSE, alpha = 0.7, outlier.shape = NA, width = 0.6, position = position_dodge(width = 0.75)) +
  geom_point(alpha = 0.8, size = 2, show.legend = FALSE, position = position_beeswarm(priority = "descending", dodge.width = 0.75, cex = 5)) +
...
library(ggbeeswarm)
ggplot(total, aes(Stock, AreaCorr, fill = Genotype)) +
  geom_violin(draw_quantiles = c("0.5"), show.legend = FALSE, trim = TRUE, alpha = 0.7, scale = "area", adjust = 2) +
  #geom_dotplot(fill = "black", color = "black", binaxis = "y", binwidth = 300, stackdir = "center", position = "dodge") +
  geom_point(alpha = 0.8, size = 2, show.legend = FALSE, position = position_jitterdodge(dodge.width = 1, jitter.width = 0.1)) +
  scale_y_continuous(labels = function(x) x / 1000) +
  labs(x = "stocks", y = expression(paste("total folded area per embryo ", (µm^2)~x~10^3))) +
  facet_grid(cols = vars(Stock), scales = "free_x") +
  scale_fill_manual(values=c("#56b4e9", "#e69f03", "#cc79a7")) +
  #scale_x_discrete(labels = c("wildtype", "heterozygote", "homozygote", "heterozygote", "homozygote")) +
  theme_classic() +
  theme(text =  element_text(size = 35),
        strip.background = element_blank(),
        panel.background = element_rect(colour = "black", size=1),
...
ggplot(results[results$Stock != "prd" & results$Type == "EF",], aes(Sample, AreaCorr, fill = Sample)) +
  geom_boxplot(show.legend = FALSE, alpha = 0.7, outlier.shape = NA, width = 0.6, position = position_dodge(width = 0.75)) +
  geom_point(alpha = 0.8, size = 2, show.legend = FALSE, position = position_beeswarm(priority = "descending", dodge.width = 0.75, cex = 2)) +
  scale_y_continuous(labels = function(x) x / 1000) +
...
```{r individual-area-per-ectopic-fold}
#ggplot(results[results$Stock != "prd",], aes(Stock, AreaCorr, fill = Type)) +
ggplot(results[results$Stock != "prd" & results$Type == "EF",], aes(Sample, AreaCorr, fill = Sample)) +
  #geom_violin(draw_quantiles = c("0.5"), show.legend = TRUE, trim = TRUE, alpha = 0.7) +
  geom_violin(draw_quantiles = c("0.5"), show.legend = FALSE, trim = TRUE, alpha = 0.7, scale = "area", adjust = 2) +
  geom_point(alpha = 0.8, size = 2, show.legend = FALSE, position = position_jitterdodge(dodge.width = 1, jitter.width = 0.1)) +
  scale_y_continuous(labels = function(x) x / 1000) +
  labs(x = "stocks", y = expression(paste("area ", (µm^2)~x~10^3))) +
...
ggplot(results, aes(Stock, AreaCorr, fill = Genotype)) +
  geom_boxplot(show.legend = FALSE, alpha = 0.7, outlier.shape = NA, width = 0.6, position = position_dodge(width = 0.75)) +
  geom_point(alpha = 0.8, size = 2, show.legend = FALSE, position = position_beeswarm(priority = "descending", dodge.width = 0.75, cex = 3)) +
  scale_y_continuous(labels = function(x) x / 1000) +
...
```{r area-individual-folds}
ggplot(results, aes(Stock, AreaCorr, fill = Genotype)) +
  geom_violin(draw_quantiles = c("0.5"), show.legend = FALSE, trim = TRUE, alpha = 0.7, scale = "area", adjust = 1) +
  geom_point(alpha = 0.8, size = 2, show.legend = FALSE, position = position_jitterdodge(dodge.width = 1, jitter.width = 0.05)) +
  scale_y_continuous(labels = function(x) x / 1000) +
  labs(x = "stocks", y = expression(paste("area of individual folds ", (µm^2)~x~10^3))) +
  scale_fill_manual(values=c("#56b4e9", "#e69f03", "#cc79a7")) +
  facet_grid(cols = vars(Position), scales = "free") +
...
ggplot(results, aes(Genotype, AreaCorr, fill = Genotype)) +
  ...
  facet_grid(~Stock) 
  facet_grid(~Stock, strip.position = "top") 
...
ggplot(results, aes(Perim., AreaCorr, colour = Stock)) + geom_point()
