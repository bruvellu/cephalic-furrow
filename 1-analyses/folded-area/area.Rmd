...
total_area_per_embryo <- aggregate(AreaCorr ~ Embryo + Stock + Genotype,
                                   area, function(x) c(
                                     count = length(x),
                                     sum = sum(x)))

# Makes column names nicer
total_area_per_embryo <- do.call(data.frame, total_area_per_embryo)
colnames(total_area_per_embryo)[colnames(total_area_per_embryo) == "AreaCorr.count"] <- "Folds.count"

# Save data frame to file
write.csv(total_area_per_embryo, file = "total_area_per_embryo.csv", row.names = FALSE)
```
...
ggplot(total_area_per_embryo, aes(Stock, AreaCorr.sum, fill = Genotype)) +
...
# Total folds and folded area per embryo
total_area_per_embryo <- aggregate(
...
# Total folds and folded area per embryo
stats_total_area_per_embryo <- aggregate(
  AreaCorr ~ Embryo + Stock + Genotype,
  area,
  function(x) c(
    count = length(x),
