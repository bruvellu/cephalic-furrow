...
colnames(totalcf) <- c("Embryo", "Length")
...
ggplot(total, aes(factor(Sample, level = c("control", "cauterized")), Length, colour = Sample)) +
...
ggplot(total, aes(Distance, Length)) + geom_point(alpha = 0.8, aes(colour = Sample)) +
  labs(title = "Total CF length by total GB distance per embryo", x = "Distance to GB", y = "Length of CF")
with(total, cor(Length, Distance))
