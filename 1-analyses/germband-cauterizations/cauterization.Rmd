...
ggplot(summary_embryos, aes(factor(Sample, level = c("control", "cauterized")), Mean, colour = Sample)) +
  geom_violin() + geom_jitter(alpha = 0.8, width = 0.15) +
  geom_pointrange(aes(ymin=Mean-SD, ymax=Mean+SD), color = "black", alpha = 0.5, data = summary_samples) +
