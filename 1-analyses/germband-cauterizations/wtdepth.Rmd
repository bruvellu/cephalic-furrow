...
ggplot(results, aes(factor(Sample, level = c("control", "cauterized")), Length, fill = Sample)) +
  ...
  geom_quasirandom(size = 2, width = 0.2, alpha = 0.8, show.legend = FALSE) +
...
ggplot(means, aes(factor(Sample, level = c("control", "cauterized")), Mean, fill = Sample)) +
  #geom_violin(draw_quantiles = c("0.5"), show.legend = FALSE, trim = TRUE, alpha = 0.7, scale = "area", adjust = 2) +
  geom_bar(stat = "summary", fun = "mean", alpha = 0.7, width = 0.5, show.legend = FALSE) +
  ...
  geom_pointrange(aes(ymin=Mean-SD, ymax=Mean+SD), position = position_quasirandom(width = 0.15, bandwidth = 0.1), show.legend = FALSE) +
