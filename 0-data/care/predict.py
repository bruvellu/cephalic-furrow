...
def parse_args():
    ...
    model.add_argument('--factor',            metavar='', type=float,    required=False,                         default=None,                                             help="upsampling factor along Z")
...
def main():
    ...
    required = ('--input-dir','--input-axes', '--norm-pmin', '--norm-pmax', '--model-basedir', '--model-name', '--factor', '--output-dir')
    for r in required:
        dest = r[2:].replace('-','_')
    ...
    K.clear_session()
    model = UpsamplingCARE(config=None, name=args.model_name, basedir=args.model_basedir)
    ...
    for file_in in tqdm(file_list, disable=args.quiet or (n_tiles is not None and np.prod(n_tiles)>1)):
        ...
        restored = model.predict(img, axes=args.input_axes, factor=args.factor, normalizer=normalizer, n_tiles=n_tiles)
